...
set.seed(123)
beta_true <- rnorm(3, 0, 1) # True item difficulty 
theta_true <- rnorm(1000, 0.5 , 1) #True latent trait 
...
logLik_rasch <- function(beta, response_matrix) {
  ...
  sum_algorithm_prob <- function(beta, score) {
    ...
    bi <- exp(-beta) # set b_i as exp(-Beta_i) for higher understandability 
...
grad_rasch <- function(beta, response_matrix) {
  ...
  sum_algorithm_prob <- function(beta, score) {
    results <- matrix(1, nrow = length(beta) + 1, ncol = length(beta) + 1) # make a results matrix filled with ones with J+1 columns and rows, where J is the number of items. 
...
beta_init <- rep(1, ncol(response_matrix)) # Start with initial beta values of 1 for each item

# Run the multivariate Newton-Raphson algorithm
beta_estimated <- multivariate_newton_raphson(beta_init, response_matrix)
scale_beta <- scale(beta_estimated) # This yields better results
