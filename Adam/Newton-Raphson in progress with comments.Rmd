...
The probability of a person $p$ getting $X$ points on an item $i$, where $X = 0,1$, under a Rasch model is given by:  

....

Where $\theta_p$ is the ability of the person and $\beta_i$ is the difficulty of the item. Here, we are interested in estimating $\beta_i$ for $n$ number of items such that $i = 1,2,...,n$. Given a set of observed data, such as the generated dataset above, this is equivalent to 
